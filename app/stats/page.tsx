...
import { getAllParties, exportAllData } from '@/lib/localDatabase'
...
export default function Stats() {
  ...
  useEffect(() => { 
    async function loadStats() {
      try {
        ...
        const validTxns = allData.transactions.filter(t => !t.isDeleted && t.transactionDate >= startOfMonth && t.transactionDate <= now.toISOString())
        totalTxns = validTxns.length
        
        for (const txn of validTxns) {
          if (txn.type === 'RECEIVED') moneyIn += txn.amount
          if (txn.type === 'GIVEN') moneyOut += txn.amount
        }

        const toTake = parties.filter(p => p.balance > 0).reduce((acc, p) => acc + p.balance, 0)
        const toGive = parties.filter(p => p.balance < 0).reduce((acc, p) => acc + Math.abs(p.balance), 0)

        const topTake = parties.filter(p => p.balance > 0).sort((a, b) => b.balance - a.balance).slice(0, 5)
        const topGive = parties.filter(p => p.balance < 0).sort((a, b) => Math.abs(b.balance) - Math.abs(a.balance)).slice(0, 5)

        setS({ toTake, toGive, moneyIn, moneyOut, totalTxns, topTake, topGive })
      } catch (err) {
        console.error(err)
      }
    }
  ...
  if (!s) return <div className="p-6 text-slate-600">Loading local stats...</div>
