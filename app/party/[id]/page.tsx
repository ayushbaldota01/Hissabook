...
import { getParty, getPartyTransactions, updateParty, deleteParty, createTransaction, updateTransaction, deleteTransaction } from '@/lib/localDatabase'
...
export default function PartyLedger() {
  ...
  const load = async () => {
    try {
      ...
      setParty(p)
      if (p) {
        const t = await getPartyTransactions(id)
        setTxns(t)
      }
    } catch (e) {
      console.error(e)
...
function EditPartyModal({ party, onClose }: { party: Party; onClose: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      await updateParty(party.id, { name, type, phone: phone || undefined, notes: notes || undefined })
    } catch (err) {
      console.error(err)
    }
    onClose()
...
function AddTxnModal({ partyId, partyType, onClose }: { partyId: string; partyType: string; onClose: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      await createTransaction(partyId, { type, amount: parseFloat(amount), transactionDate: date, description: desc || undefined })
    } catch (err) {
      console.error(err)
    }
    onClose()
...
function EditTxnModal({ txn, partyType, onClose }: { txn: Txn; partyType: string; onClose: () => void }) {
  ...
  const del = async () => {
    ...
    try {
      if (txn.id) await deleteTransaction(txn.id)
    } catch (err) {
      console.error(err)
    }
    onClose()
